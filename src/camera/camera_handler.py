...
class CameraHandler:
    ...
    def __init__(self, camera_index: int = 0, resolution: Tuple[int, int] = (640, 480), controls: Optional[dict] = None, still_settle_ms: int = 700):
        """
    ...
    def start(self) -> bool:
        ...
        if self.use_picamera2 and PICAMERA2_AVAILABLE:
            try:
                ...
                try:
                    ...
                    if modes:
                        self.full_still_size = max(modes, key=lambda m: m['size'][0] * m['size'][1])['size']
                except Exception:
                    self.full_still_size = None

                # Configure preview
                config = self.picam2.create_preview_configuration(main={"size": self.resolution, "format": "RGB888"})
                ...
                if frame is not None and frame.size > 0:
                    self.is_open = True
                    logger.info(f"✓✓✓ Camera started successfully with Picamera2! Resolution: {frame.shape}")
                    return True
                else:
        ...
        try:
            ...
            self.is_open = True
            logger.info(f"Camera {self.camera_index} started with OpenCV ({success_count}/{max_attempts} frames)")
            return True
    ...
    def capture_still_array(self, size: Optional[Tuple[int, int]] = None) -> Optional[np.ndarray]:
        ...
        try:
            ...
            still_config = self.picam2.create_still_configuration(main={"size": still_size, "format": "RGB888"})
        ...
        try:
            ...
            if isinstance(mode, str) and mode.lower() == 'manual':
                try:
                    ...
                    logger.warning(f"Invalid manual exposure/gain values, skipping manual exposure: {conv_err}")
            else:

...
class CameraHandler:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            for i in range(max_attempts):
                try:
                    ...
                    if ret and frame is not None:
                        success_count += 1
                        logger.debug(f"Warmup frame {i+1}: Success, shape={frame.shape}")
                    else:
                        logger.debug(f"Warmup frame {i+1}: Failed (read returned {ret})")
                    time.sleep(0.2)
            ...
            if success_count == 0:
                logger.error(f"Camera {self.camera_index} opened but cannot read frames after {max_attempts} attempts. This is typically a driver/configuration issue. See CAMERA_TROUBLESHOOTING.md")
                self.cap.release()
                self.cap = None
                return False
            
            self.is_open = True
            logger.info(f"Camera {self.camera_index} started successfully ({success_count}/{max_attempts} warmup frames)")
            return True

...
class SyncQueueManager:
    ...
    def add_to_queue(self, record_type: str, record_id: int, data: Dict, priority: int = 0) -> bool:
        """
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (record_type, record_id, data_json, priority, datetime.now().isoformat()))
    ...
    def mark_attendance_synced(self, attendance_id: int, cloud_record_id: str = None) -> bool:
    ...
    def update_device_status(self, device_id: str = None, sync_count: int = None) -> bool:
    ...
    def clear_old_failed_records(self, max_retries: int = 5) -> int:
        ...
        try:
            ...
            if deleted > 0:
                logger.warning(f"Removed {deleted} failed sync records (max retries exceeded)")
            
            return deleted
        
        except Exception as e:
            logger.error(f"Error clearing old failed records: {e}")
            return 0

...
class FaceQualityChecker:
    ...
    def __init__(self, config: dict = None):
        ...
        # Quality thresholds
        self.min_face_size = config.get('min_face_size', 80)  # Face width minimum (adjusted for typical camera distance)
        self.center_tolerance_x = config.get('center_tolerance_x', 0.12)  # 12% of width
        self.center_tolerance_y = config.get('center_tolerance_y', 0.12)  # 12% of height
        ...
        self.mouth_cascade = cv2.CascadeClassifier(cascade_path + 'haarcascade_smile.xml')
        
        logger.info(f"Face quality checker initialized with thresholds: "
                   f"face_size={self.min_face_size}px, center_tol={self.center_tolerance_x*100}%, "
                   f"pose=±{self.max_yaw}/±{self.max_pitch}/±{self.max_roll}°")
    
    def check_quality(self, frame: np.ndarray, face_box: Tuple[int, int, int, int]) -> Dict:
        """
        ...
        if h <= 0 or w <= 0:
            return {'passed': False, 'score': 0.0, 'checks': {}, 'reason': 'Invalid face box'}
        
        face_roi = frame[y:y+h, x:x+w]
        
        if face_roi.size == 0:
            return {'passed': False, 'score': 0.0, 'checks': {}, 'reason': 'Invalid face ROI'}
    ...
    def _check_face_centered(self, frame: np.ndarray, face_box: Tuple[int, int, int, int]) -> Dict:
    ...
    def _check_head_pose(self, frame: np.ndarray, face_box: Tuple[int, int, int, int]) -> Dict:
        ...
        passed = abs(yaw) <= self.max_yaw and abs(pitch) <= self.max_pitch and abs(roll) <= self.max_roll
    ...
    def _check_mouth_closed(self, face_roi: np.ndarray) -> Dict:
        ...
        mouths = self.mouth_cascade.detectMultiScale(mouth_region, 1.5, 5, minSize=(25, 15))
    ...
    def _check_brightness(self, face_roi: np.ndarray) -> Dict:
        ...
        deviation = abs(avg_brightness - ideal_brightness) / (ideal_brightness - self.min_brightness)
...
class AutoCaptureStateMachine:
    ...
    def __init__(self, quality_checker: FaceQualityChecker, stability_duration: float = 3.0, timeout: float = 15.0):
        """
        ...
        self.reset()
        logger.info(f"Auto-capture state machine initialized: stability={stability_duration}s, timeout={timeout}s")
    ...
    def update(self, frame: np.ndarray, face_box: Optional[Tuple[int, int, int, int]]) -> Dict:
        """
        ...
        if not quality_result['passed']:
            ...
            if self.stability_start_time is not None:
                logger.debug(f"Quality check failed: {quality_result['reason']} - resetting timer")
            self.stability_start_time = None
            return {
                'state': 'WAITING',
                'should_capture': False,
        ...
        if stability_elapsed >= self.stability_duration:
            ...
            if not self.capture_triggered:
                ...
                self.state = 'CAPTURED'
                logger.info(f"Capture triggered after {stability_elapsed:.1f}s of stability")
                return {
                    'state': 'CAPTURED',
                    'should_capture': True,
                    'elapsed': elapsed,

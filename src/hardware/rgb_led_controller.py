...
class RGBLEDController:
    ...
    def _initialize_gpio(self):
        ...
        try:
            ...
            logger.info(
                f"RGB LED initialized on GPIO pins R:{self.gpio_pins['red']}, "
                f"G:{self.gpio_pins['green']}, B:{self.gpio_pins['blue']}"
            )
        except ImportError:
            logger.warning(
                "RPi.GPIO not available - RGB LED disabled (software-only mode)"
            )
            self.enabled = False
    ...
    def set_color(
        self, red: int, green: int, blue: int, brightness: Optional[float] = None
    ):
        """
    ...
    def show_color(self, color_name: str, fade: bool = True, blocking: bool = False):
        ...
        if blocking:
            ...
            thread = threading.Thread(
                target=self._show_color_pattern, args=(color, fade)
            )
            thread.daemon = True
                ...
                try:
                    ...
                    for _ in range(cycles):
                        ...
                        for i in range(steps):
                            brightness = i / steps
                            self.set_color(
                                *color, brightness=brightness * self.brightness
                            )
                            time.sleep(step_delay)

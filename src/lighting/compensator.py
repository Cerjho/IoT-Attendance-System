...
class LightingCompensator:
    ...
    def compensate(self, frame: np.ndarray, lighting_analysis: Dict) -> np.ndarray:
        ...
        if apply_low_light:
            ...
            compensated = self._apply_low_light_enhancement(compensated, lighting_analysis)
        ...
        if self.auto_brightness:
            ...
            if brightness < 80 or brightness > 200:
                logger.debug(f"Applying brightness adjustment (current: {brightness:.1f})")
    ...
    def _apply_low_light_enhancement(self, frame: np.ndarray, lighting_analysis: Dict) -> np.ndarray:

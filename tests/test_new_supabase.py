    ...
    if response.status_code == 200:
        ...
        if students:
            ...
            for i, student in enumerate(students[:5], 1):
                ...
                print(f"      Grade: {student.get('grade_level')} | Section: {student.get('section')}")
    ...
    if response.status_code == 200:
        ...
        if records:
            ...
            for record in records:
                print(f"   • Date: {record.get('date')} | Time: {record.get('time_in')} | Status: {record.get('status')}")
    else:
        print(f"❌ FAILED: HTTP {response.status_code}")
        print(f"Response: {response.text[:200]}")
except Exception as e:
    ...
    if students and len(students) > 0:
        ...
        if response.status_code == 200:
            ...
            if result:
                ...
                print(f"✅ SUCCESS: Student number '{student_number}' → UUID '{student_uuid}'")

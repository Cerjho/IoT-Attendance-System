...
def main():
    ...
    auto_capture = AutoCaptureStateMachine(quality_checker, stability_duration=3.0, timeout=15.0)
    ...
    try:
        ...
        camera = CameraHandler({'camera': {'index': 0, 'resolution': {'width': 640, 'height': 480}}})
        camera.start()
    ...
    try:
        while True:
            ...
            if testing and largest_face:
                ...
                cv2.putText(display_frame, f"State: {status['state']}", (10, 30),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                cv2.putText(display_frame, status['message'], (10, 60),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                
                # Display countdown
                if status.get('countdown'):
                    countdown = status['countdown']
                    cv2.putText(display_frame, str(countdown), (display_frame.shape[1]//2 - 50, display_frame.shape[0]//2),
                ...
                cv2.putText(display_frame, "Waiting for face...", (10, 30),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            
            else:
                cv2.putText(display_frame, "Press SPACE to start test", (10, 30),

...
This script tests all SMS templates including the new enhancements.
"""
...
def send_test_sms(student_id):
    ...
    try:
        ...
        if success:
            ...
            print(
                f"Good day! {name} (ID: {sid}) checked IN at {time_str} on {date_str}."
            )
            if attendance_url:
    ...
    while True:
        ...
        if choice == "1":
            ...
            if student_info and student_info[2]:
                confirm = (
                    input(f"Send SMS to {student_info[2]}? (y/n): ").strip().lower()
                )
                if confirm == "y":
